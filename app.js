...
function printFibonacciTimeouts(limit) {
    ...
    function printNext() {
      ...
      //enclose the setTimeout call in a variable, so you can stop it later
      //it wouldn't stop otherwise, and may cause a memory leak
...
class DigitalClock {
  ...
  class AlarmClock extends DigitalClock {
    ...
    //we cant use dom because we are not in the browser, we are in node, running scripts.
    //and also if I did It wasnt linked to your html, so it wouldnt work.
    //please double check your code before you submit it.
    update() {
      const parts = this.getTimeParts();
      const minuteFormatted = parts.minute.toString().padStart(2, "0");
      const timeFormatted = `${parts.hour}:${minuteFormatted}`;
  ...
  //we cant use dom because we are not in the browser, we are in node, running scripts
  const clockObject = new AlarmClock("08:00");
  clockObject.start();
...
//Good job on most parts.


//pointers:
//but note that we are not using dom in this exercises, so you dont need to display it to the browser.
//also and even If I did, it wasnt linked to the html file, so it wouldnt work.
//-put the timeout in a variable if you are going to call next, because it wont stop the next function from running. this will cause a memory leak.
//-submit the code in english please. Wouldn't mind the comments but the code should be in english.
